{
	// map()数据的映射
	let json = [ { title:"es6",status:0},{ title:"node",status:0},{ title:"wechat",status:1},{ title:"vue",status:0} ];
	let result = json.map(function(item){
		/* return {
			name:item.title,
...
// reduce(function(返回值，currentVal,currentIndex,array){},[]/{})
{
	...
	// console.log(result)
	// isArray
	let list = [1,2,["2nd",3,4,["3th",5,6]],[7,8]];
